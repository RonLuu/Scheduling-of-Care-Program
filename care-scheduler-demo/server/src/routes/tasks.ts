...
tasks.get('/_debug-counts', (_req, res) => {
  const q = (sql: string) => (dbg.prepare(sql).get() as any).n as number;
  res.json({
...
tasks.get('/my-tasks', ensureAuth, async (req: any, res) => {
...
tasks.post('/occurrences/:id/complete', ensureAuth, async (req: any, res) => {
