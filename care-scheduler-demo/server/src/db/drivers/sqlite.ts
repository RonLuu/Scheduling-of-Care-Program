...
import type { Repo, TaskRow, DbUserWithHash, DbUser, Role } from '../repo.js';
...
let hasPerson = false;
try {
  const countRow = db.prepare('SELECT COUNT(*) as n FROM person').get() as { n: number };
...
if (!hasPerson) {
  ...
  function addOccurrences(id: number | bigint, unit:'DAY'|'WEEK'|'MONTH'|'YEAR', interval:number, monthsSpan=6) {
    const end = new Date(); end.setMonth(end.getMonth()+monthsSpan);
    let cursor = new Date();
    while (cursor <= end) {
      addOcc.run(Number(id), cursor.toISOString().slice(0,10), 'DUE' ,staff);
      if (unit === 'DAY') cursor.setDate(cursor.getDate()+interval);
      else if (unit === 'WEEK') cursor.setDate(cursor.getDate()+7*interval);
...
export const sqliteRepo: Repo = {
  ...
  const stmt = db.prepare(`
    ...
    ORDER BY due_on ASC
  `);
  return stmt.all(days) as TaskRow[];
  },
  ...
  async getUserByEmail(
    email: string
  ): Promise<{ id: number; name: string; email: string; role: 'ADMIN'|'STAFF'|'READONLY'; password_hash: string } | null> {
    const r = db.prepare(
      `SELECT id, name, email, role, password_hash FROM user WHERE email = ?`
    ).get(email) as
      | { id: unknown; name: unknown; email: unknown; role: unknown; password_hash: unknown }
      | undefined;
    ...
    return {
      ...
      role: r.role as 'ADMIN'|'STAFF'|'READONLY',
  ...
  async getUserById(
    ...
    const r = db.prepare(
      ...
      | { id: unknown; name: unknown; email: unknown; role: unknown }
      | undefined;
  ...
  async createUser(u: { name: string; email: string; role: Role; password_hash: string }): Promise<number> {
    const stmt = db.prepare(
