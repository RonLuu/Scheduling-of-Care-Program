...
export const pgRepo = {
    ...
    async completeOccurrence(occId, userId, comment) {
        ...
        try {
            ...
            await client.query(`INSERT INTO completion_log (occurrence_id, completed_by, comment)
         VALUES ($1, $2, $3)`, [occId, userId, comment ?? null]);
            await client.query(`UPDATE occurrence SET status = 'COMPLETED' WHERE id = $1`, [occId]);
    ...
    async getUserByEmail(email) {
        const { rows } = await pool.query(`SELECT id, name, email, role, password_hash
         FROM "user"
        WHERE email = $1
        LIMIT 1`, [email]);
        return rows[0] ?? null;
    ...
    async createUser(u) {
        const { rows } = await pool.query(`INSERT INTO "user" (name, email, role, password_hash)
       VALUES ($1, $2, $3, $4)
       RETURNING id`, [u.name, u.email, u.role, u.password_hash]);
